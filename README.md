...
[![HyperNick](https://img.shields.io/badge/HyperNick-%E5%85%BC%E5%AE%B9-brightgreen)](https://github.com/Shallow-Y/HyperNick)

Bukkit 服务端插件 — 作为 [Hyper Heartrate](https://github.com/ChaYeWuu/Hyper-Heartrate) Fabric 模组的服务端中转，接收并转发玩家心率数据，支持 PlaceholderAPI 变量，兼容 [HyperNick](https://github.com/Shallow-Y/HyperNick) 匿名身份伪装。
...
- Java 25+
- [PlaceholderAPI](https://www.spigotmc.org/resources/placeholderapi.6245/) — 必需，变量扩展 + HyperNick 桥接依赖
- 客户端需安装 [Hyper Heartrate](https://github.com/ChaYeWuu/Hyper-Heartrate) Fabric 模组

### 可选

- [HyperNick](https://github.com/Shallow-Y/HyperNick) — 匿名身份伪装兼容

## 安装

1. 从 [Releases](../../releases) 下载最新版 `HyperHeart-Bukkit-*.jar`
2. 放入服务端的 `plugins/` 目录
3. 重启服务端
...
本插件自动兼容 [HyperNick](https://github.com/Shallow-Y/HyperNick)（原 NickPlus）匿名身份伪装插件。

当玩家使用 `/nick` 匿名后，HyperNick 会为其生成一个 Fake UUID（v5 格式），客户端看到的是伪装身份。本插件在 S2C 广播时自动检测并替换为 Fake UUID，确保：
...
```
HyperNick ──→ PlaceholderAPI ──→ HyperHeart-Bukkit
  (提供 %hypernick_fakeuuid%)    (通过 PAPI 桥接读取)
```

无需额外配置，安装 PlaceholderAPI 和 HyperNick 后自动生效。
...
```bash
git clone https://github.com/ChaYeWuu/HyperHeart-Bukkit.git
cd HyperHeart-Bukkit
./gradlew build
```

产物位于 `build/libs/HyperHeart-Bukkit-*.jar`
...
└── integration/
    └── HyperNickBridge.java            # HyperNick Fake UUID 兼容桥接
```
...
- [HyperNick](https://github.com/Shallow-Y/HyperNick) — 匿名身份伪装插件